...
canvas.DrawModHeader(font, "Content Exporter", "by Shockah", SKTextAlign.Right);

using var image = surface.Snapshot();
using var data = image.Encode(SKEncodedImageFormat.Png, 100);
using var stream = File.OpenWrite(Path.Combine(GetRootPath(), "..", "ContentExporter", "Banner.png"));
data.SaveTo(stream);
