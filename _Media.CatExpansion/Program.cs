...
var screenshot1 = SKImage.FromEncodedData(Path.Combine(GetRootPath(), "assets", "Background.png"));

canvas.Clear(SKColors.Empty);
canvas.DrawImage(screenshot1, new SKRect(0, 0, imageInfo.Width, imageInfo.Height));
...
canvas.DrawModHeader(font, "CAT Expansion", "by Arin, Kurio and Shockah", SKTextAlign.Right);

using var image = surface.Snapshot();
using var data = image.Encode(SKEncodedImageFormat.Png, 100);
using var stream = File.OpenWrite(Path.Combine(GetRootPath(), "..", "CatExpansion", "Banner.png"));
data.SaveTo(stream);
