...
var bannerBackground = SKImage.FromEncodedData(Path.Combine(GetRootPath(), "assets", "BannerBackground.png"));

canvas.Clear(SKColors.Empty);
canvas.DrawImage(bannerBackground, 0, 0);
...
canvas.DrawModHeader(font, "Custom Run Options", "by Shockah", SKTextAlign.Right);

using var image = surface.Snapshot();
using var data = image.Encode(SKEncodedImageFormat.Png, 100);
using var stream = File.OpenWrite(Path.Combine(GetRootPath(), "..", "CustomRunOptions", "Banner.png"));
data.SaveTo(stream);
