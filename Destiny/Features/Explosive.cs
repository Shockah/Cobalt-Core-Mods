...
internal sealed class Explosive : IRegisterable
{
	...
	private static readonly Pool<OnExplosiveTriggerArgs> OnExplosiveTriggerArgsPool = new(() => new());
	...
	public static void Register(IPluginPackage<IModManifest> package, IModHelper helper)
	{
		...
		ModEntry.Instance.Helper.Events.RegisterAfterArtifactsHook(nameof(Artifact.OnPlayerPlayCard), (Card card, State state, Combat combat) =>
		{
			...
			var attack = new AAttack { damage = card.GetDmg(state, GetExplosiveDamage(state, combat, card)) };
			combat.Queue(attack);
			ModEntry.Instance.Helper.Content.Cards.SetCardTraitOverride(state, card, ExplosiveTrait, false, permanent: false);

			OnExplosiveTriggerArgsPool.Do(args =>
			{
				args.State = state;
				args.Combat = combat;
				args.Card = card;
	...
	private sealed class OnExplosiveTriggerArgs : IDestinyApi.IHook.IOnExplosiveTriggerArgs
	{
		public State State { get; set; } = null!;
		public Combat Combat { get; set; } = null!;
		public Card? Card { get; set; }
		public AAttack AttackAction { get; set; } = null!;
	}
}