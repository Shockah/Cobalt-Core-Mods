...
internal sealed class SurroundedWeaponPerk : IRegisterable
{
	public static ICardTraitEntry Trait { get; private set; } = null!;
	
	public static void Register(IPluginPackage<IModManifest> package, IModHelper helper)
	{
		var icon = ModEntry.Instance.Helper.Content.Sprites.RegisterSprite(ModEntry.Instance.Package.PackageRoot.GetRelativeFile("assets/CardTraits/WeaponPerks/Surrounded.png"));
		
		Trait = ModEntry.Instance.Helper.Content.Cards.RegisterTrait("Surrounded", new()
		{
			Icon = (_, _) => icon.Sprite,
			Name = ModEntry.Instance.AnyLocalizations.Bind(["CardTrait", "WeaponPerk", "Surrounded", "Name"]).Localize,
			Tooltips = (_, _) =>
			[
				new GlossaryTooltip($"cardtrait.{ModEntry.Instance.Package.Manifest.UniqueName}::Surrounded")
				{
					Icon = icon.Sprite,
					TitleColor = Colors.cardtrait,
					Title = ModEntry.Instance.Localizations.Localize(["CardTrait", "WeaponPerk", "Surrounded", "Name"]),
					Description = ModEntry.Instance.Localizations.Localize(["CardTrait", "WeaponPerk", "Surrounded", "Description"]),
				}
			]
		});
	...
	private static bool AreShipsFullyOverlapping(State state)
	{
		if (state.route is not Combat combat)
			return false;
		if (state.ship.x <= combat.otherShip.x && state.ship.x + state.ship.parts.Count >= combat.otherShip.x + combat.otherShip.parts.Count)
			return true;
		if (combat.otherShip.x <= state.ship.x && combat.otherShip.x + combat.otherShip.parts.Count >= state.ship.x + state.ship.parts.Count)
			return true;
		return false;
	}
