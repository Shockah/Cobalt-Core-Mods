...
// 			return
// 			[
// 				.. StatusMeta.GetTooltips(Stack.TetrisStatus.Status, Math.Max(ship.Get(Stack.TetrisStatus.Status), 1)),
// 				.. Thing.GetTooltips(),
// 			];
// 		}
//
...
// 			var amount = ship.Get(Stack.TetrisStatus.Status);
//
// 			if (amount <= 0)
// 				return;
//
...
//
// 			c.QueueImmediate([
// 				new AStatus { targetPlayer = FromPlayer, mode = AStatusMode.Set, status = Stack.TetrisStatus.Status, statusAmount = 0 },
// 				.. Enumerable.Range(0, amount).Select(_ => new ASpawn { fromPlayer = FromPlayer, thing = Mutil.DeepCopy(thing), timer = totalTime / amount, fromX = rng.NextInt() % ship.parts.Count }.SetStacked()),
// 			]);
// 		}
// 	}
//
